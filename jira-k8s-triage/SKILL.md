...
3. 直接給答案；答不了的指出正確文件或負責團隊
4. 常見問題註明「建議沉澱為 FAQ」
5. 用「Ask Platform 範本」產草稿；分級一律 S4
...
6. 用「Pull Request Review 範本」產草稿；範本的「Diff 證據」欄位必填，
   填不出具體檔名與行數 = 草稿無效，禁止進入收尾
7. **絕不碰 approve 按鈕**——approve 永遠由人工在 PR 平台執行
8. 瀏覽器連續失敗 2-3 次就停下回報，不要無限重試
...
2. 判斷回覆類型並對應處理（可能複合，逐項回應）：
   ...
   - **回報止血結果** → 有效：建議觀察期後結案；無效或惡化：依原報告的升級觸發
     條件升級，提高分級並改分流
