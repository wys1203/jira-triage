...
3. 依序對每張單執行「單張處理流程」；**每張處理完，輸出一行摘要
   （單號／結論／是否發布）後即拋棄該單細節再讀下一張**——長佇列
   不得把每張單的完整內容都留在腦中
4. 新單處理完後，用設定區的 Follow-up JQL 掃描複診佇列；`updated` 會被任何
   欄位異動觸發，須逐張以 `jira-state.sh` 確認是否真為 FOLLOWUP_PENDING，
   是才進路徑 4
...
2. `jira-state.sh <KEY>` 判斷分診狀態，依輸出直接分流（邏輯已封裝在
   script 內——水位線 = 最後一則含 footer 的 comment，不需自行推理）：
   - `UNTRIAGED` → 未分診，繼續往下
   - `FOLLOWUP_PENDING` → 已分診且有新回覆，直接進入路徑 4（複診），
     跳過步驟 5 的類型分派
