...
4. 逐項處理證據：
   - 圖片（png/jpg/gif/jpeg）：`jira-attach.sh <附件ID>` 下載後用 Read tool
     視覺分析，依 `references/analysis.md` 的「圖片判讀準則」抄錄關鍵錯誤
...
2. **先天條件與政策檢查**：對照 `references/preconditions.md` 與
   ...
   （node name、namespace、資源設定等）。政策違規時改用「Policy 違規
   回覆範本」產草稿（不分流、不動 priority，assignee 改指回 Reporter），
   直接進收尾。
...
2. 對照 `references/preconditions.md` 與 `references/policies.md`（依各條
   ...
   該條「標準回覆」作答並引用規則編號；政策違規改用「Policy 違規回覆範本」
...
6. **Policy 檢核**：對照 `references/policies.md` 適用於 PR 的政策
   ...
   不做後續判讀結論，改用「Policy 違規回覆範本」產草稿（引用規則編號與
   具體證據），直接進收尾（不動 priority，assignee 改指回 Reporter）；
   報案者拆單重提後由複診接手
