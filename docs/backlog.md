...
## 規則庫維護防護（2026-07-16 規劃）

**背景**：skill 已上線，日常由弱模型協助新增案例/規則，但弱模型思考
...
- [ ] **B-12 規則維護三層防護** —
  1. **Authoring 指南** `references/authoring.md`：分類決策樹
     ...
     標準回覆語氣約束（引用「語氣與對外原則」）、完整 worked example
  2. **`rule-lint.sh`**：機械驗證規則庫結構不變量——編號連續不重複、
     必要欄位標籤齊全、SKILL.md 引用的 `##` 節標題契約未破壞、
     footer 字串未被改動、`[~username]` 格式；掛進 tests/run-all.sh。
     與 B-3 同 lint 思路（B-3 驗 comment 草稿輸出、B-12 驗規則庫本身），
     實作時可共用骨架
  3. **流程閘門**：弱模型只能產出固定格式的「規則變更提案」
     （分類判定+依據+完整條文+影響檔案清單），經使用者核可 + lint
     通過才落地——「未經核可不寫 Jira」鐵則的規則庫版
  - 關聯：B-8 防執行時引用錯規則，B-12 防維護時寫錯規則
